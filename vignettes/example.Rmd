...
**Note:** Permutation Importance calculation is pretty computationally intensive and may increase the time to finish the analysis run. As a general recommendation it should be used if there is no computational resources constraints. 
...
GeneSelectR workflow is highly customizable depending on your needs. For example, if you wish to add any other feature selection methods from sklearn, you should pass it as a named list like this:
...
Finally, we can pass it as an arguments to fit_and_evaluate_pipelines function:
...
Same can be done with classifiying estimator. For example you want to use XGBoost classifier instead of default random forest:
...
![Alt text](../vignettes/images/plot_metrics.png) Or access it as a dataframe:
...
The format argument can be set to three possible options: (1) 'ensembl_id' (eg. ENSGXXXXXXX), (2) 'gene_name' (eg. TSPAN) or combination of both separated by a separating string (eg. ENSGXXXXXXX\_\_TSPAN). The resulting AnnotatedGeneLists class object contains following fields:
...
There is a wrapper function to run GO enrichment analysis with [clusterprofiler](https://guangchuangyu.github.io/software/clusterProfiler/) package. To run GO enrichment with default setting simply run:
...
This will produce a dataframe containing information on number of children terms of specified terms, number of all the terms and fraction of children terms related to the parent one in the total number of identified GO terms (without p-value filtering):
...
Final step is the analysis is the clustering and semantic similarity analysis of GO Terms from every list. This is done via [simplifyEnrichment R package](https://github.com/jokergoo/simplifyEnrichment). For the convenience of data input, there is a wrapper for [simplifyGOFromMultipleLists()]() function implemented:
