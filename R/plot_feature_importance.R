...
plot_feature_importance <- function(pipelineresults, top_n_features = 10) {
  ...
  for(method_name in names(pipelineresults@mean_feature_importances)){
    ...
    # Create the plots
    p1 <- ggplot2::ggplot(top_mean_features, ggplot2::aes(x = reorder(feature, mean_importance), y = mean_importance)) +
    ...
    p2 <- ggplot2::ggplot(top_permutation_features, ggplot2::aes(x = reorder(feature, mean_importance), y = mean_importance)) +
      ggplot2::geom_bar(stat = "identity", aes(fill = color)) +
      ggplot2::scale_fill_identity() +
      ggplot2::coord_flip() +
      ggplot2::theme_minimal() +
...
#
#     p2 <- ggplot2::ggplot(top_permutation_features, ggplot2::aes(x = reorder(feature, mean_importance), y = mean_importance)) +
#       ggplot2::geom_bar(stat = "identity", position = "dodge", fill = "lightgreen") +
#       ggplot2::coord_flip() +
#       ggplot2::theme_minimal() +
#       ggplot2::labs(x = "Features", y = "Importance", title = paste("Permutation Feature Importance for", method_name))
#
